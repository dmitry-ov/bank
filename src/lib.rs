...
impl Bank {
    ...
    pub fn increase_account(&mut self, account: String, amount: u32) -> Result<(), BankError> {
        if !self.accounts.contains(&account) {
            return Err(AccountAlreadyExists(
                format!(
                    "Account {} does not exist", account)));
    ...
    pub fn decrease_account(&mut self, account: String, amount: u32) -> Result<(), BankError> {
        if !self.accounts.contains(&account) {
            return Err(AccountAlreadyExists(
                format!(
                    "Account {} does not exist", account)));
